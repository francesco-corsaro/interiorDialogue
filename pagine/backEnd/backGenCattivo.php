...
if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
    ...
    if ($uploadOk == 0) {
        ...
        header("location: http://mytraining.altervista.org/interiorDialogue/pagine/caricaGenitoreBuono.php");
        // if everything is ok, try to upload file
    } else {
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            echo "The file ". basename( $_FILES["fileToUpload"]["name"]). " has been uploaded. check4 <br>";
            include 'dbUtility/insert_path_cattivo.php';
            
            insert_path_cattivo($_SESSION['utente'],basename($_FILES["fileToUpload"]["name"]));
